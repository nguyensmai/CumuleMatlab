...
for iPred=1:nPred
 ...
 pred(6) = FFN([51], [6], 10, inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(7) = FFN([1 2], [7], 10, inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(8) = FFN([1 ], [8], 10, inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(9) = FFN([51], [9], 10, inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(10) = FFN([51 52], [10], 10, inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(11) = FFN([51], [11], [20 20], inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(12) = FFN([1 2], [12], [20 20], inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(13) = FFN([1 ], [13], [20 20], inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(14) = FFN([51], [14], [20 20], inputsSet,1);  % good for env4 [s1 m1] -> s1
 pred(15) = FFN([51 52], [15], [20 20], inputsSet,1);  % good for env4 [s1 m1] -> s1
